<?if(!$B_PROLOG_M) die();?>
<?
function insert_db($table,$arRes){
	$res = "INSERT INTO {name_t} ( {key} ) VALUES ( {val} )";	
	foreach($arRes as &$v){
		$v = "\"".$v."\"";	
	}
	unset($v);
	$key = implode(" , ",array_keys  ($arRes));
	$val = implode(" , ",array_values($arRes));
	...
	return 	str_replace($healthy, $yummy, $res);
}
function update_db($table,$arRes,$id){
	$res = "UPDATE {name_t} SET {query} WHERE ID = {id}";
	$items = array();
