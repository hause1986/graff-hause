	...
	public static function GetList(
		...
		if(
			...
			$sqlFilterstr = '
				...
				FROM
			';
			foreach( $arFilter['PROP'] as $keyPr => $valPr ){
				$res = CIBlock::MkOperationFilter( $keyPr );
				$resOperand = CIBlock::Operand( $res["OPERATION"] );
				...
				$sqlFilter[] =  '
					(
						...
						FROM
							...
							`h_prop_iblock`
						WHERE
								h_prop_iblock.ID_IBLOCK = ' . intval( $arFilter['ID_IBLOCK'] ) . '
							AND h_prop_iblock.CODE = \'' . $res['FIELD'] . '\'
							AND	h_prop_iblock.ID = h_prop_value.ID_PROP
							AND h_prop_value.VALUE ' . $resOperand . ' ' . $valPr . '
					) as h' . $i ;
					
				$i++;
			}
			$sqlFilterstr .= implode( ', ', $sqlFilter );			
			if( count( $sqlFilter ) > 1 ){
				$sqlFilterstr .= ' WHERE  ( 1 = 1 ) ';
				$step = 1;
				for( $i = 1; $i < count( $sqlFilter ); $i++ ){					
					$sqlFilterstr .= ' AND ( h' . $step . '.ID = h' . intval( $step + 1 ) . '.ID )';
					$step++;
				}				
			}
			//echo $sqlFilterstr;
		...
		if( strlen( $sqlFilterstr ) ){
			$query = '
				SELECT 
					h_iblok_elements.ID				as ID,
					h_iblok_elements.NAME			as NAME,
					h_iblok_elements.SORT			as SORT,
					h_iblok_elements.CODE			as CODE,
					h_iblok_elements.ACTIVE			as ACTIVE,
					h_iblok_elements.ID_IBLOCK		as ID_IBLOCK,
					h_iblok_elements.CREATE_DATE	as CREATE_DATE,
					h_iblok_elements.UP_DATE		as UP_DATE					
				FROM  
					h_iblok_elements,
					( ' . $sqlFilterstr .' ) as prop
				WHERE 
						( 1 = 1 ) 
					AND ( h_iblok_elements.ID = prop.ID )' 
					. $strSqlSearch . $strSqlOrder . $lim;
		}else{
